...
- When **m** signs or encrypts data, **m** always uses the latest keys, denoted by **[0]K<sub>m</sub>** and **[0]K<sub>C</sub>** (as newly issued keys are inserted on the left side of a list).
...
- Each transaction residing in **𝓛<sub>C</sub>** contains a serialization of `EntryCrypt` ("entry **e**"):
```
...
```
- Given entry **e** arriving from **𝓛<sub>C</sub>**, **e**`.HeaderCrypt` is decrypted using **[]K<sub>C</sub>** into an `EntryHeader` ("**e<sub>hdr</sub>**"):
```
type EntryHeader struct {
    ...
    ChannelEpochID    UUID     // Cites latest epoch of the channel in effect when this entry was authored
...
- **𝓡<sub>i</sub>** is constructed from:

```
// ChannelID is used to specify a channel in C's virtual channel space
type ChannelID UUID 
...
type ChannelStore struct {
    ...
    EntryTable        []EntryIndex    // Partial entry headers indexed by TimeAuthored and hashname
    ContentTome       DataTome        // Store containing entry content (referenced by EntryIndex) 
}

// EntryIndex packages the the essential parts of an entry, plus status information.
type EntryIndex struct {
    EntryStatus      EntryStatus     // Enum specifying status of this entry (e.g. LIVE, DEFERRED)
    EntryHeader      EntryHeader     // Essential parts of this entry's header
    ContentPos       uint64          // Specifies an offset into ..ContentTome
    ContentLen       uint32          // Specifies a byte length at .ContentPos in ..ContentTome
}
...
- Entries that do not conform to channel properties or permissions are "deferred" within **𝓡<sub>i</sub>**. The node periodically reattempts to merge these entries since later-arriving entries may alter **𝓡<sub>i</sub>** such that previously deferred or rejected entries now validate. 
...
- Members of **C** maintain their copy of the community keyring, **[]K<sub>C</sub>**, where:
...
- **C**'s [member epoch channel](#member-epoch-channel) contains each member's UUID, current crypto "epoch", and other root information:
```
...
```
- **C**'s [root access control channel](#Root-Access-Control-Channel) (ACC) is a log containing access grants to member ID
...
## Channels
Channels are intended to be used for any purpose, and are the "client service" ultimately afforded to members (through channel handlers running in their client).  To that end, the system itself uses channels for specialized purposes in order to operate,  Policies 

1. **Reserved channels** are special channels used to specify root-level information and permissions, namely admin and member records and key exchange. 
    - These channels are special in that entries must meet additional security checks/requirements an explicit purpose.  
...
2. **General purpose channels** are the principal purpose of this entire system.
    ...
    - General purpose channels can either be:
        ...
        - **private**, where entry content is encrypted with the specified by **e<sub>hdr</sub>**`.ContentKeyID`.  
            - Key mechanics for private channels are similar to [starting a new community epoch](#Starting-a-New-Community-Epoch), except the channel owner updating the `ChannelEpoch` performs key generation and distribution.  
...
- This is a special community-public channel where members post revisions to their published `MemberEpoch` record.  
    - A `MemberEpoch` contains essential information about a specific member, such as their most recently published public keys or their "home" channel `UUID`
- In addition to members posting updates to their 
...
- Given the permissions and prerequisites are met on **C** are met to bestow member status to actor **α**:
    ...
    5. On a newly created "blank" node, **n<sub>α</sub>** (or an existing node of **C** in a logged-out state)
        ...
        - Once that **𝓡<sub>α</sub>** is up to date (i.e. with **𝓔<sub>α0</sub>** live in the _Member Epoch Channel_), **α** immediately posts a successor `MemberEpoch`, just as they would when [starting a new member epoch](#Starting-a-New-Member-Epoch).  
