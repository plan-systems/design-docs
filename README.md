...
| `/plan/ch/file/pdf`  |            `ipfs`\|`binary`         | The client UI represents this channel as a single monolithic object. Tapping on it causes the most recent channel entry (interpreted as the latest revision) to be fetched and opened locally on the client using a PDF viewing application.  Power users can open and review earier revisions. |
...
| `/plan/ch/feed/rss`  |                 `xml`               | This channel is used to publish a sequence of text, audio, or video items with accompanying meta elements (e.g. title, link, and description).  This channel's epoch properties house [RSS](https://en.wikipedia.org/wiki/RSS) channel elements, and each PLAN channel entry is xml and corresponds to a RSS `item` element.  |
