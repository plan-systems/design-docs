...
- You are rightly unsettled if you think all the data in PLAN community is stored on its append-only shared storage layer.  The PDI is generally implemented as an immutable transaction log or blockchain, and so we would be remiss to assume that the PDI is sufficient to meet a community's storage needs.
